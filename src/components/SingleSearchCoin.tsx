					...
					<tr key={index} className='group hover:bg-base-200'>
						<td>
							<Link to={`/coin/${coin.uuid}`} onClick={() => setInputValue('')}>
								<div className="flex items-center gap-3">
									...
									<div>
										<div className="text-xs sm:text-sm font-bold group-hover:text-mainColor">{coin.name}</div>
						...
						<td className=" md:max-w-7">
							<div className="md:flex justify-between">
								<p className="mb-1">{(Number(coin.price) < 0.99) ? <span>{Number(coin.price).toFixed(4)}</span>: <span>{Number(coin.price).toFixed(2)}</span>}</p>
